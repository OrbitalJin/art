...
interface IntervalContextValues {
  skip: () => void;
...
const IntervalContext = createContext<IntervalContextValues | null>(null);

interface Props {
  children: ReactNode;
}

export const IntervalContextProvider: React.FC<Props> = ({ children }) => {
...
export const useIntervalTimer = () => {
  ...
  if (!context) {
    throw new Error(
      "useIntervalTimer must be wrapped in a IntervalContextProvider",
    );
  }
